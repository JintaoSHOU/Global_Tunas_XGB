...
var_importance_1 = function(shap_result, top_n=10, col='grey')
{
  var_importance=tibble(var=names(shap_result$mean_shap_score), importance=shap_result$mean_shap_score)
  
  var_importance=var_importance[1:top_n,]
  maxV = max(var_importance$importance)
  
  ggplot(var_importance, aes(x=reorder(var,importance), y=importance)) + 
    geom_bar(stat = "identity", fill=col) + 
    geom_rect(aes(ymin=0, ymax=maxV, xmin=40.5, xmax=50.5), colour='black', fill=NA, linetype=2)+
    coord_flip() + 
    theme_light() + 
    theme(axis.title.y=element_blank()) 
}

var_importance_2 = function(shap_result, top_n=10, col='grey')
{
  var_importance=tibble(var=names(shap_result$mean_shap_score), importance=shap_result$mean_shap_score)
...
var_importance_3 = function(shap_result, top_n=10, col='grey', ocean)
{
  var_importance=tibble(var=names(shap_result$mean_shap_score), importance=shap_result$mean_shap_score)
...
var_importance_4 = function(shap_result, col='grey', ocean)
{
  var_importance=tibble(var=names(shap_result$mean_shap_score), importance=shap_result$mean_shap_score)
  var_importance=var_importance[1:5,]
  var_importance=var_importance %>% mutate(importance = importance/sum(importance))
...
var_importance_12 = function(shap_result, top_n=50, col='grey')
{
  var_importance=tibble(var=names(shap_result$mean_shap_score), importance=shap_result$mean_shap_score)
  var_importance=var_importance[1:top_n,]
  maxV = max(var_importance$importance)
  a = sort(var_importance$importance, decreasing = T)
  ggplot(var_importance, aes(x=reorder(var,importance), y=importance)) + 
    geom_bar(stat = "identity", fill=col, alpha=0.5) + 
    geom_line(aes(x=50:1, y=a))+
    #geom_rect(aes(ymin=0, ymax=maxV, xmin=40.5, xmax=50.5), colour='black', fill=NA, linetype=2)+
    coord_flip() + 
    theme_light() + 
    theme(axis.title.y=element_blank()) 
